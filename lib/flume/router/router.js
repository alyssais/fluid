...
var expressHandler = function(req, res) {
	...
	var viewPath = "app/views/" + this.controller + "/" + this.action + ".html.handlebars";

	Fluid.application.readFile(viewPath, function(error, view) {
		res.send(Handlebars.compile(view.toString())(data));
	});
};
